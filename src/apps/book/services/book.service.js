...
function removeReview(bookId, reviewId) {
	return get(bookId).then(currBook => {
...
function getBooksFromGoogle(searchWord) {
	return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchWord}`).then(res => {
		const shortBooks = res.data.items.slice(0, 5)
		const books = shortBooks.map(book => prepareData(book))
		return books
	})
}

function prepareData(bookFromGoogle) {
	const {
		volumeInfo: { categories, authors, title, subtitle, description, language, pageCount, publishedDate },
	} = bookFromGoogle
	...
	const book = {
		...
		publishedDate,
		pageCount,
		listPrice: { amount: utilService.getRandomIntInclusive(0, 1000), currencyCode: 'USD' },
	}
	return book
}

function addGoogleBook(bookFromGoogle) {
	return save(bookFromGoogle)
}
